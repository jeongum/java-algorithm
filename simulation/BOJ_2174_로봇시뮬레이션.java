...
public class BOJ_2174_로봇시뮬레이션 {
    static int[] di = {-1, 0 , 1, 0}; // N,W,S,E
    static int[] dj = {0, -1 , 0, 1}; // N,W,S,E
    static int A, B;
    static int N, M;
    static int[][] robots;
    static int[][] isThere;
    public static void main(String[] args) throws Exception{
        ...
        for(int i = 0 ; i < M ; i++){
            ...
            for(int j =0 ; j < cnt ; j++) {
                switch (inst) {
                    case 'L': {
                        robots[robot][2] = (robots[robot][2] + 1) % 4;
                        break;
                    }
                    case 'R': {
                        robots[robot][2] = (robots[robot][2] - 1)<0 ? 3:(robots[robot][2] - 1);
                        break;
                    }
                    case 'F': {
